...
for lease in 10 20 # Variation of lease time
do
	echo "Themis $lease"
	python scripts/generate_cluster_config.py --racks 2 --machines 2 --slots 2 --gpus 4 --policy Themis --fairness_knob 0 --visibility_knob 1 --lease_time $lease --config_file config_themis_lease$lease-d0-e1
	mv configuration/cluster/config_themis_lease$lease-d0-e1.json configuration/cluster/$1/ #Move config to separate file
	cluster_config_file=`echo config_themis_lease$lease-d0-e1.json`
	workload_config_file=configuration/jobs/final_workload.json
	mvn exec:java -Dexec.mainClass="com.wisr.mlsched.Simulation" -Dexec.args="configuration/cluster/$1/$cluster_config_file $workload_config_file" > results/$1/themis_lease$lease-d0-e1.txt 
done

# Generate cluster configurations for other configurations
#for lease in 1 2 3 5 8 10 20 40
#do
