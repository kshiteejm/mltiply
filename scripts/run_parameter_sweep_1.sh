...
#for lease in 10 20 40 80 # Variation of lease time
#do
#	for d in 0 1 2 4 8 # Variation of fairness knob
#	do
#		for e in 0.2 0.4 0.6 0.8 1 # Variation of visibility knob
#		do
#			echo "Themis $lease $d $e"
#			python scripts/generate_cluster_config.py --racks 2 --machines 4 --slots 2 --gpus 4 --policy Themis --fairness_knob $d --visibility_knob $e --lease_time $lease --config_file config_themis_lease$lease-d$d-e$e
...
for lease in 10 #5 10 20 40 80
do
	echo "Other Schemes $lease"
	python scripts/generate_cluster_config.py --racks 2 --machines 4 --slots 2 --gpus 4 --policy Themis --lease_time $lease --config_file config_themis_lease$lease
	python scripts/generate_cluster_config.py --racks 2 --machines 4 --slots 2 --gpus 4 --policy SLAQ --lease_time $lease --config_file config_slaq_lease$lease
	python scripts/generate_cluster_config.py --racks 2 --machines 4 --slots 2 --gpus 4 --policy Gandiva --lease_time $lease --config_file config_gandiva_lease$lease
	python scripts/generate_cluster_config.py --racks 2 --machines 4 --slots 2 --gpus 4 --policy Tiresias --lease_time $lease --config_file config_tiresias_lease$lease
	mv configuration/cluster/config_themis_lease$lease.json configuration/cluster/$1/
	...
	workload_config_file=configuration/jobs/trace_workload.json
	mvn exec:java -Dexec.mainClass="com.wisr.mlsched.Simulation" -Dexec.args="configuration/cluster/$1/config_themis_lease$lease.json $workload_config_file" > results/$1/themis_lease$lease.txt &
